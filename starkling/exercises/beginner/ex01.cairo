...
func light_star{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (address) = get_caller_address()
    let (dust_am) = dust.read(address)
    assert_le(dust_amount, dust_am + 1)
    
    let (next_av_slot) = slot.read(address)
    dust.write(address, dust_am - dust_amount)
    star.write(address, next_av_slot, dust_amount)
    slot.write(address, next_av_slot + 1)
    a_star_is_born.emit(address, next_av_slot, dust_amount)
    return ()
end
